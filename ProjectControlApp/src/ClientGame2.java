...
public class ClientGame2 extends JFrame {
	Image monsterimg;
	Image imback;
	Image Character;
	GameImg panel;
	...
	public GameImg() {
		...
		Character= toolkit.getImage("images/jingjing.png");
		//이미지 리사이징
	...
	protected void paintComponent(Graphics g) {
		...
		for (int i = 0 ; i < bullets.size(); i++) { // bullets.size() == 3
			// 계산하는 부분
			if(bullets.get(i).isMoving) bullets.get(i).y -= 3;
			else {
	...
	public Bullet(int x,int y){
	 	...
	 	this.y = y;
			...
			Toolkit toolkit= Toolkit.getDefaultToolkit();
			attack = toolkit.getImage("images/hee.png");
			attack = attack.getScaledInstance(50, 45, Image.SCALE_SMOOTH);
