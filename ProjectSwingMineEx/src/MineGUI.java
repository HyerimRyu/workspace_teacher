...
public class MineGUI extends JPanel implements ActionListener {
	...
	public MineGUI(Level lv) {
		// level값에 따른 ROWS, COLS, MINE 값을 정해주고 그걸 이용한 게임패널을 생성한다.
		if (lv == Level.ROW) {
	...
	public int[][] configureMine() { 		// 지뢰를 랜덤으로 배치하고 지뢰주위 숫자 세팅을 담당.
		int count = 0;
	...
	public boolean validRange(int r, int c) { 		// 넘어온 좌표값의 범위가 벗어났는지 담당.(true, false)
			...
			try {
				...
				if (text == null) updateScore();
					...
					if ( Integer.parseInt( score[0] ) > time ) updateScore();
						...
						text = "아쉽습니다. 기록을 갱신하지 못했습니다. 최고기록은 " + score[0] + " 입니다.";
	...
	public void updateScore() {
		String name = JOptionPane.showInputDialog( this, "최고기록을 갱신하셨습니다. \n 이름을 입력해 주세요", "잇힝~!!", JOptionPane.OK_CANCEL_OPTION );
		String text = null;
		String score = null;
		try {
			...
			score = temp[0] + "초 " + temp[1] + "콩그레츄레이션~ㅡ.ㅡ잇힝~축하";
      		JOptionPane.showMessageDialog( this, score, "End", JOptionPane.OK_OPTION , new ImageIcon("new4.gif") );
			} catch (IOException ex) { System.err.println("update오류 발생");}
	}
	/* timer action method */
	public void actionPerformed(ActionEvent e) {
		// time을 자릿수별로 구한다음 각 Lebel에 image Icon setting
